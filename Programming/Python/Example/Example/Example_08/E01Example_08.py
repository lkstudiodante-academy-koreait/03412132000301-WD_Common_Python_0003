...
def start(args):
	...
	단, 리스트는 데이터가 추가되는 과정에서 해당 데이터를 저장하기 위한 공간이 할당되기 때문에 잘못 된 인덱스 번호를
	명시하면 프로그램에 예외가 발생한다.
	
	따라서 insert 함수로 데이터를 추가 할 때 사용 가능한 인덱스 번호의 범위는 0 ~ 리스트 길이 이다.
	
	Ex)
	oList = [1, 2, 3]
	oList.insert(3, 4)          <- "[1, 2, 3, 4]" 와 같이 마지막 위치에 데이터 추가
	"""
